...
console.log("main.js : loading start");

console.log("requirejs config : start");
requirejs.config({
/*
	baseUrl:
...
console.log("requirejs config : end");
...
require( [
	...
	function (text, $, angular) {
		console.log("main.js / require() : start");
		//이 함수는 위에 명시된 모든 디펜던시들이 다 로드된 뒤에 호출된다.
		//주의해야할 것은, 디펜던시 로드 완료 시점이 페이지가 완전히 로드되기 전 일 수도 있다는 사실이다.
		
		//페이지가 완전히 로드된 뒤에 실행
		$(document).ready(function () {
			console.log("main.js / require() / documentReady ~");
			//위의 디펜던시 중 myApp이 포함된 app.js가 로드된 이후에 아래가 수행된다.
			//임의로 앵귤러 부트스트래핑을 수행한다.
			angular.bootstrap(document, ['myApp']);
			console.log("myApp Module bootstrapping success!");
			
		});
		console.log("main.js / require() : end");
	}
);
console.log("main.js : loading end");